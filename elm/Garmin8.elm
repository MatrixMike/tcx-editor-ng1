...
tcxDecoder =
    ...
    at ["TrainingCenterDatabase"] <| xtract "Activities" activitiesDecoder
...
trackDecoder = xtract "Track" <| ("Trackpoint" := list tpDecoder)

tpDecoder : Decoder Trackpoint
tpDecoder =
    object3 Trackpoint
        (xtract "Time" string)
        (xtract "Position" posDecoder)
        (xtractFloat "DistanceMeters")
...
-- xtract s d = tuple1 identity (s:= list d)

xtractFloat: String -> Decoder Float
xtractFloat str = customDecoder (xtract str string) String.toFloat
