...
Elm.Garmin = Elm.Garmin || {};
Elm.Garmin.make = function (_elm) {
   "use strict";
   _elm.Garmin = _elm.Garmin || {};
   if (_elm.Garmin.values)
   return _elm.Garmin.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   ...
   $String = Elm.String.make(_elm),
   $Task = Elm.Task.make(_elm);
   var viewTrack = function (tp) {
      return $Html.div(_L.fromArray([]))(_L.fromArray([function ($) {
         return $Html.text($Basics.toString($));
      }(tp.distance)]));
   };
   var viewLap = function (lap) {
      return $Html.div(_L.fromArray([]))(A2($List._op["::"],
      A2($Html.p,
      _L.fromArray([]),
      _L.fromArray([$Html.text(A2($Basics._op["++"],
      "Distance: ",
      $Basics.toString(lap.distance)))])),
      A2($List.map,
      viewTrack,
      lap.tps)));
   };
   ...
   var lapDecoder = A4($Json$Decode.object3,
   Lap,
   ...
   $Basics.identity,
   activitiesDecoder));
   var loadData = $Effects.task($Task.map(Raw)($Task.toMaybe(A2($Http.get,
   tcxDecoder,
   "http://localhost:5000/longexample.json"))));
   var init = {ctor: "_Tuple2"
              ,_0: i
              ,_1: loadData};
   _elm.Garmin.values = {_op: _op
                        ,Position: Position
                        ...
                        ,activitiesDecoder: activitiesDecoder
                        ,activityDecoder: activityDecoder
                        ,lapDecoder: lapDecoder
                        ,trackDecoder: trackDecoder
                        ,tpDecoder: tpDecoder
                        ,posDecoder: posDecoder
                        ,xtractFloat: xtractFloat
                        ,update: update
                        ,viewTrack: viewTrack
                        ,viewLap: viewLap
                        ,view: view};
